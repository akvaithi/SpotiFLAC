...
func (a *App) GetSpotifyMetadata(req SpotifyMetadataRequest) (string, error) {
	...
	data, err := backend.GetFilteredSpotifyData(ctx, req.URL, req.Batch, time.Duration(req.Delay*float64(time.Second)), separator, func(tracks interface{}) {
		emitEvent("metadata-stream", tracks)
	})
	if err != nil {
...
func (a *App) DownloadTrack(req DownloadRequest) (DownloadResponse, error) {
	...
	if req.Service != "flacit" {
		return DownloadResponse{
			Success: false,
			Error:   fmt.Sprintf("unknown service: %q (only the Telegram gateway is supported)", req.Service),
...
func (a *App) SelectFolder(defaultPath string) (string, error) {
	_ = defaultPath
	return "", fmt.Errorf("native folder picker is not available in the web version; use the file browser")
}

func (a *App) SelectFile() (string, error) {
	return "", fmt.Errorf("native file picker is not available in the web version; use the file browser")
}
...
func (a *App) GetFlacItStatus() map[string]interface{} {
	...
	if err != nil {
		return map[string]interface{}{"ok": false, "error": err.Error(), "gateway_url": gatewayURL}
	}
	defer resp.Body.Close()
	...
	if err := json.Unmarshal(body, &status); err != nil {
		return map[string]interface{}{"ok": false, "error": "invalid gateway response", "gateway_url": gatewayURL}
	}
...
func (a *App) ExportFailedDownloads() (string, error) {
	...
	path := filepath.Join(serverDownloadDir(), defaultFilename)
...
func (a *App) DownloadFFmpeg() DownloadFFmpegResponse {
	emitEvent("ffmpeg:status", "starting")
	err := backend.DownloadFFmpeg(func(progress int) {
		emitEvent("ffmpeg:progress", progress)
	})
	if err != nil {
		emitEvent("ffmpeg:status", "failed")
	...
	emitEvent("ffmpeg:status", "completed")
	return DownloadFFmpegResponse{
		Success: true,
...
func (a *App) InstallFFmpegWithBrew() InstallFFmpegWithBrewResponse {
	emitEvent("ffmpeg:status", "Installing FFmpeg via Homebrew...")
	err := backend.InstallFFmpegWithBrew(func(progress int, status string) {
		emitEvent("ffmpeg:progress", progress)
		emitEvent("ffmpeg:status", status)
	})
	if err != nil {
		emitEvent("ffmpeg:status", "failed")
...
func (a *App) SelectAudioFiles() ([]string, error) {
	return nil, fmt.Errorf("native file picker is not available in the web version; use the file browser")
}
